            ...
            if(command.split(" ").length == 2 || command.split(" ").length == 3) {
                if(command.split(" ")[1].equalsIgnoreCase("com/anomaly/user") || command.split(" ")[1].equalsIgnoreCase("username")) {
                    if(command.split(" ").length != 3) {
                        Console.println("§redInvalid use of command §light_greyunban com.anomaly.user§red. Try §light_greyunban com.anomaly.user (username)§red.", false, Color.RED);
                        return;
                    }
                ...
                Console.println("§redInvalid use of command §light_greyunban§red. Try §light_greyunban com.anomaly.user (username)§red.", false, Color.RED);
            }
        }
    }
}
